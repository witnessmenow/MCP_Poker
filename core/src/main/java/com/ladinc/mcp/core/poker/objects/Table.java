...
import com.ladinc.mcp.core.poker.actions.BetAction;
...
public class Table {
    ...
    private void doBettingRound() {
        ...
        while (playersToAct > 0) {
            ...
            if (actor.isAllIn()) {
                ...
                Set<PokerAction> allowedActions = getAllowedActions(actor);
                ...
                if (!allowedActions.contains(action)) {
                    if (action instanceof BetAction && !allowedActions.contains(PokerAction.BET)) {
                        throw new IllegalStateException(String.format("Player '%s' acted with illegal Bet action", actor));
                    } else if (action instanceof RaiseAction && !allowedActions.contains(PokerAction.RAISE)) {
     ...
     */
    private Set<PokerAction> getAllowedActions(Player player) {
        Set<PokerAction> actions = new HashSet<PokerAction>();
        if (player.isAllIn()) {
